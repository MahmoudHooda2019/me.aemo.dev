    ...
    <main>
      ...
      <section class="particle-section section-wrapper relative">
        ...
        <div 
          ...
          @keydown.enter.space.prevent="handleBadgeClick(badge.id)"
        >
...
const badgeTimers = new Map<string, ReturnType<typeof setTimeout>>()

const handleBadgeClick = (badgeId: string) => {
  // Clear existing timer for this badge if any
  const existing = badgeTimers.get(badgeId)
  if (existing) clearTimeout(existing)
  
  // Toggle: if already expanded, collapse immediately
  if (expandedBadges.value.has(badgeId)) {
    expandedBadges.value.delete(badgeId)
    badgeTimers.delete(badgeId)
    return
  }

  // Add expanded class, auto-collapse after 2.5 s
...
const isBadgeExpanded = (badgeId: string): boolean =>
  expandedBadges.value.has(badgeId)
...
.particle-section {
  ...
  padding: 2rem;
  /*
    FIX: Do NOT add isolation: isolate here — it would create a new stacking
    context that traps the badges below the fixed header elements.
    Instead, keep z-indexes consistent at the document level.
  */
}
...
.floating-badge {
  ...
  padding: 16px 20px;
  /*
    FIX: z-index reduced from an implied high value to 30 so it sits
    below the fixed header/theme-toggle (z-index 10 000+).
    The header elements now own the top of the stack.
  */
  z-index: 30;
  ...
  /* FIX: added cursor pointer — badges are interactive */
  cursor: pointer;
  /* FIX: smooth press feedback */
  transition:
...
@keyframes float {
  0%, 100% { transform: var(--badge-base-transform, translateX(40%)) translateY(0px);    }
  50%       { transform: var(--badge-base-transform, translateX(40%)) translateY(-10px); }
}
...
.floating-badge:active {
  transform: var(--badge-base-transform, translateX(40%)) scale(0.96) !important;
}

/* Keyboard focus ring */
.floating-badge:focus-visible {
  outline: 2px solid var(--accent-primary, #6ee7b7);
  outline-offset: 3px;
}
...
.floating-badge:hover .floating-badge-text  { color: var(--accent-primary); }
.floating-badge:hover .floating-badge-subtext { color: var(--text-primary); }
...
.badge-home-right-top:hover { transform: translate(0, 0); }
...
.badge-home-right-center:hover { transform: translate(0, -50%); }
...
.badge-home-right-bottom:hover { transform: translate(0, 0); }
...
.badge-home-left-top:hover { transform: translate(0, 0); }
...
.badge-home-left-center:hover { transform: translate(0, -50%); }
...
.badge-home-left-bottom:hover { transform: translate(0, 0); }

/* ─── Intermediate breakpoint ─────────────────────────────────────── */
@media (max-width: 1100px) {
  .badge-home-right-top,
  .badge-home-right-center,
  .badge-home-right-bottom { right: 2%; }

  .badge-home-left-top,
  .badge-home-left-center,
  .badge-home-left-bottom  { left: 2%; }
}
...
.section-wrapper .grid-pattern { z-index: 0; }
...
@media (max-width: 768px) {
  ...
  /* Badges only appear in the hero, never in other section wrappers */
  .section-wrapper .floating-badge { display: none !important; }
  ...
  .particle-section .floating-badge {
    ...
    /*
      FIX: z-index set to 200 (was 9999).
      Theme toggle is z-index: 10001, mobile trigger: 10010 — always on top.
      200 is still above normal page content (z-index 1).
    */
  ...
  /* RIGHT badges — peek icon-width from right edge */
  .badge-home-right-top,
  .badge-home-right-center,
  .badge-home-right-bottom {
    right: 0 !important;
  ...
  /* LEFT badges — icon faces inward (visible peeking side) */
  .badge-home-left-top,
  .badge-home-left-center,
  .badge-home-left-bottom {
    left: 0 !important;
  ...
  /* Center badges keep translateY(-50%) in their combined transform */
  .badge-home-right-center {
